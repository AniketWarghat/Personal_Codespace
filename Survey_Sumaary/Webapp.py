...
from db import init_db, load_dataframe, get_recent_syncs
...
# DATABASE LOAD
# --------------------------------------------------
raw_df, file_name, imported_at = load_dataframe()

if raw_df is None or raw_df.empty:
    st.warning("No survey data available in the database yet. Run the scraper first.")
    st.stop()

st.sidebar.markdown("---")
st.sidebar.subheader("Database Sync Status")

recent_syncs = get_recent_syncs()
...
if imported_at:
    try:
        last_updated = pd.to_datetime(imported_at).strftime("%d %b %Y %H:%M:%S")
    except Exception:
...
st.caption(
    ...
    f"Source file: {file_name} | Last imported: {last_updated}"
)
...
with tabs[0]:
    ...
    most_common_vehicle = (
        ...
        if "3.Vehicle Type" in filtered_df.columns and not filtered_df["3.Vehicle Type"].dropna().empty else "N/A"
    )
    date_range_label = (
        f"{filtered_df['Date'].min().strftime('%d %b')} – {filtered_df['Date'].max().strftime('%d %b %Y')}"
        if "Date" in filtered_df.columns and not filtered_df["Date"].dropna().empty else "N/A"
    )
    peak_hour = "N/A"
    if "start_hour" in filtered_df.columns and filtered_df["start_hour"].notna().any():
        peak = int(filtered_df["start_hour"].value_counts().idxmax())
        peak_hour = f"{peak:02d}:00 – {peak:02d}:59"

    data_quality_score = (
        round(filtered_df["location_clean"].mean() * 100, 1)
        if len(filtered_df) and "location_clean" in filtered_df.columns else 0.0
    )
    ...
    with col_l:
        ...
        if hourly.empty:
            st.info("No hourly survey data available for the selected filters.")
        else:
            fig_h = px.bar(
                hourly,
                x="start_hour",
            ...
            fig_h.update_traces(
                hovertemplate="Hour %{x}:00 — %{y} surveys<extra></extra>"
            )
    ...
    with col_r:
        ...
        if daily.empty:
            st.info("No daily trend data available for the selected filters.")
        else:
            fig_d = px.line(
                daily,
                x="Date",
...
with tabs[2]:
    ...
    with col2:
        if not vs.empty:
            ...
            st.info("No vehicle summary available.")
    ...
    if not sv.empty:
        fig_sv = px.bar(
            ...
            title="Vehicle Type per Surveyor",
            barmode="stack",
        ...
        st.info("No surveyor-wise vehicle data available.")
    ...
    if not av.empty:
        fig_av = px.bar(
            ...
            y="Count",
            color="3.Vehicle Type",
            title="Vehicle Type per Arm / Direction",
            barmode="stack",
...
with tabs[3]:
    ...
    if not top_orig.empty:
        ...
        fig_orig.update_traces(
            hovertemplate="Origin: %{y}<br>Count: %{x}<extra></extra>"
        )
    ...
    if not top_dest.empty:
        ...
        fig_dest.update_traces(
            hovertemplate="Destination: %{y}<br>Count: %{x}<extra></extra>"
        )
...
with tabs[4]:
    ...
    if selected_surveyor:
        ...
        si3.metric("Vehicle Types", ind["3.Vehicle Type"].dropna().nunique())
        si4.metric("Arms Covered", ind["2.Arm details"].dropna().nunique())
        ...
        with col_a:
            ...
            if ind_daily.empty:
                st.info("No daily survey data available for this surveyor.")
            else:
                fig_ind_d = px.bar(
                    ind_daily,
                    x="Date",
        ...
        with col_b:
            ...
            if ind_veh.empty:
                st.info("No vehicle type data available for this surveyor.")
            else:
                fig_ind_v = px.pie(
                    ind_veh,
                    names="Vehicle Type",
        ...
        if ind_hourly.empty:
            st.info("No hourly survey distribution available for this surveyor.")
        else:
            fig_ind_h = px.bar(
                ind_hourly,
                x="start_hour",
...
st.markdown("---")
st.caption(
    f"File: {file_name} | Dashboard built for DC513MH04 survey — EEH (JJ Junction), Mumbai"
)